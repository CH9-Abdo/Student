...
class LoginWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.offline_btn = QPushButton("Work Offline (No Account)")
        self.offline_btn.setStyleSheet("""
            QPushButton {
                background-color: #f1f5f9; color: #475569; padding: 12px; 
                border-radius: 8px; font-weight: bold; margin-top: 10px;
            }
            QPushButton:hover { background-color: #e2e8f0; }
        """)
        self.offline_btn.clicked.connect(self.handle_offline)
        layout.addWidget(self.offline_btn)

        layout.addStretch()

    def handle_offline(self):
        user = self.auth.work_offline()
