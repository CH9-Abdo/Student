...
class StudentProApp {
    ...
    refreshDashboard() {
        ...
        const overallVal = get('overall-progress-val');
        if (overallVal) overallVal.textContent = `${perc}%`;

        const overallBar = get('overall-progress-bar');
        if (overallBar) overallBar.style.width = `${perc}%`;

        const exam = db.getNextExamInfo();
        const nextExamVal = get('next-exam-val');
        if (nextExamVal) nextExamVal.textContent = exam ? `${exam.name} (${exam.days}d)` : "None";

        const streakVal = get('streak-val');
        if (streakVal) streakVal.textContent = `${db.getStudyStreak()} Days`;

        // Daily goal progress
        const dailyGoal = get('daily-goal-val');
        const dailyBar = get('daily-goal-bar');
        if (dailyGoal && db.data?.user_profile) {
            const sessions = db.data.user_profile.total_sessions || 0;
            const goal = 3;
            const todaySessions = Math.min(sessions % goal || 0, goal);
            dailyGoal.textContent = `${todaySessions}/${goal}`;
            if (dailyBar) dailyBar.style.width = `${(todaySessions / goal) * 100}%`;
        }

        const container = get('todo-container');
        if (container) {
            container.innerHTML = '';
            const todos = db.getTodoChapters();
            if (todos.length === 0) {
                container.innerHTML = '<p class="mute" style="text-align:center; padding:20px 0;">All caught up! 🎉</p>';
            } else {
                todos.slice(0, 5).forEach(t => {
                    ...
                    item.innerHTML = `
                        <div>
                            ...
                            <p class="mute" style="font-size:12px; margin-top:2px;">${t.subject_name} — ${!t.video_completed ? 'Course' : 'Exercises'}</p>
                        </div>
                        <span class="badge" style="background:var(--primary-light); color:var(--primary);">${!t.video_completed ? '📖' : '✍️'}</span>
                    `;
